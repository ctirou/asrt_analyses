...
for network in tqdm(networks):
    ...
    for subject in subjects:
        ...
        for block in range(1, 24):
            ...
            tpoi = int(np.where(np.isclose(times, 0.37, atol=0.005))[0])
            pattern.append(np.load(pfname)[tpoi])
            random.append(np.load(rfname)[tpoi])
        
        if subject == 'sub05':
            pat_bsl = np.load(res_path / "pat-4.npy")[tpoi] if network in networks[:-3] else np.load(res_path / "pat-4-4.npy")[tpoi]
            rand_bsl = np.load(res_path / "rand-4.npy")[tpoi] if network in networks[:-3] else np.load(res_path / "rand-4-4.npy")[tpoi]
            for i in range(3):
...
### Pattern ###
# Sig from GAMM
seg_df = pd.read_csv(FIGURES_DIR / "TM" / "em_segments_preact_tr_pat_rand_source.csv")
...
sig_df = pd.read_csv(FIGURES_DIR / "TM" / "smooth_preact_tr_pat_rand_source.csv")
...
seg_df = pd.read_csv(FIGURES_DIR / "TM" / "em_segments_preact_tr_cont_source.csv")
...
for _, row in seg_df.iterrows():
    ...
    arr[row["start"]:row["end"] + 1] = True
    sig_dict[row["network"]] = arr
sig_df = pd.read_csv(FIGURES_DIR / "TM" / "smooth_preact_tr_cont_source.csv")
...
for i, (network, contrast_idx) in enumerate(zip(networks, ['C', 'F', 'I', 'L', 'O', 'R', 'U', 'X', 'AA', 'AD'])):
    ...
    # if msig[i]:
    #     axes[contrast_idx].fill_between(times[win], -3, -1.8, facecolor=cmap[i])
    if contrast_idx == 'C':
...
fig.savefig(figures_dir / "timeg-preact-diag.pdf", transparent=True)
...
fig.savefig(figures_dir / "timeg-preact-corr.pdf", transparent=True)
