...
def process_subject(subject, jobs):
    ...
    for block in blocks:
        ...
        random_pending = [
            region
            for region in regions
            if overwrite or not op.exists(res_paths[region] / f"rand-{block}.npy")
        ]
        if random_pending:
            ...
            for region in random_pending:
                ...
                if not labels:
                    print(f"No labels found for region {region} in subject {subject}; skipping random block {block}")
                    continue
        ...
        pattern_pending = [
            region
            for region in regions
            if overwrite or not op.exists(res_paths[region] / f"pat-{block}.npy")
        ]
        if pattern_pending:
            stcs_train, ytrain, stcs_test, ytest = get_train_test_blocks_htc(
            ...
            for region in pattern_pending:
                labels = [label for label in label_tc_train.keys() if region in label]
                if not labels:
                    print(f"No labels found for region {region} in subject {subject}; skipping pattern block {block}")
                    continue
