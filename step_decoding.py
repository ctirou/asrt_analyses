...
for trial_type in trial_types:
    ...
    for subject in subjects:
        ...
        for train_0, test_0 in cv.split(X_0, y_0):
            ...
            scores_0.append(np.array(clf.score(X_0[test_0], y_0[test_0])).mean(0))
        ...
        for i, score_list in zip(range(1, len(all_epo)), [scores_1, scores_2, scores_3, scores_4]):
            if trial_type == 'all':
                ...
                for train, test in cv.split(X, y):
                    score_list.append(np.array(clf.score(X[test], y[test])).mean(0))
                ...
                for train, test in cv.split(X_pat, y_pat):
                    score_list.append(np.array(clf.score(X_pat[test], y_pat[test])).mean(0))
            else:
                # random
                ...
                for train, test in cv.split(X_rdm, y_rdm):
                    score_list.append(np.array(clf.score(X_rdm[test], y_rdm[test])).mean(0))
    ...
    for score, label in zip([scores_0, scores_1, scores_2, scores_3, scores_4], ['train_prac', 'test_b1', 'test_b2', 'test_b3', 'test_b4']): 
        plt.subplots(1, 1, figsize=(16, 7))
        plt.plot(times, score.mean(0), label=label)
        pval = decod_stats(score - chance)
        sig = pval < .05
        plt.fill_between(times, chance, score.mean(0), where=sig, alpha=.3)
        plt.legend()
        plt.axhline(y=0.25, ls="dashed", color="k", alpha=.2)
        plt.axvline(x=0, ls="dashed", color="k", alpha=.2)
        plt.savefig(op.join(figures, trial_type, "f'{label}.png"))
        plt.close()