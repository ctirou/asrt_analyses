...
for subject in tqdm(subjects):
    ...
    for i, behav_session in enumerate(behav_sessions):
        ...
        if i == 0:
            behav.columns = [col for col in behav.columns if col not in ['isi_if_correct', 'isi_if_incorrect']] + [''] * len(['isi_if_correct', 'isi_if_incorrect'])

        # remove incorrect trials
        behav = behav[behav['talalat'] == 1].reset_index(drop=True)
        behav['session'] = i
        behav_df.append(behav)
        
    behav_df = pd.concat(behav_df, ignore_index=True)

    behav_df = pd.concat(behav_df).reset_index(drop=True)
