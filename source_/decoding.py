...
del epochs
# get len label names
labels = mne.read_labels_from_annot(subject='sub01', parc='aparc', hemi=hemi, subjects_dir=subjects_dir)
...
for subject in subjects:
    ...
    for trial_type in trial_types:
        ...
        for session_id, epo_fname in zip(range(5), ['practice', 'b1', 'b2', 'b3', 'b4']):
            # get session behav and epoch
            if session_id == 0:
                epo_fname = 'prac'
            else:
                epo_fname = 'sess-%s' % (str(i).zfill(2))
