...
figure_dir = data_path / 'figure_results'
ensure_dir(figure_dir / lock)

# get labels
labels = read_labels_from_annot(subject='sub01', parc='aparc', hemi='both', subjects_dir=FREESURFER_DIR, verbose=verbose)

for ilabel, label in enumerate(labels):
    # load patterns and randoms time-generalization on all epochs
    patterns, randoms = [], []
    for subject in tqdm(subjects[:1]):
        pattern = np.load(res_dir / 'pattern' / f"{subject}-all-scores.npy")
        patterns.append(pattern)
        random = np.load(res_dir / 'random' / f"{subject}-all-scores.npy")
