...
def process_subject(subject, network, jobs, verbose):
        ...
        for block in blocks:
            ...
            for i, (_, test_index) in enumerate(kf.split(ypat)):
                ...
                if not op.exists(res_path / f"pat-{epoch_num}-{block}-{i+1}.npy") or overwrite:
                    print(f"{network} - Computing Mahalanobis for quarter {i+1} for {subject} epoch {epoch_num} block {block} pattern")
                    ...
                    print(f"{network} - Mahalanobis for bin {i+1} for {subject} epoch {epoch_num} block {block} pattern already exists")
            ...
            for i, (_, test_index) in enumerate(kf.split(yrand)):
                ...
                if not op.exists(res_path / f"rand-{epoch_num}-{block}-{i+1}.npy") or overwrite:
                    print(f"{network} - Computing Mahalanobis for quarter {i+1} for {subject} epoch {epoch_num} block {block} random")
                    ...
                    print(f"{network} - Mahalanobis for bin {i+1} for {subject} epoch {epoch_num} block {block} random already exists")
...
if is_cluster:
    ...
    jobs = 1
    Parallel(-1)(delayed(process_subject)(subject, network, jobs, verbose) for subject in subjects for network in networks)