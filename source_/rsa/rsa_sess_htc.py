...
def process_subject(subject, jobs):
    ...
    for region in ['Hippocampus', 'Thalamus', 'Cerebellum-Cortex']:
        ...
        if not op.exists(res_dir / "rand-prac.npy") or overwrite:
            ...
            np.save(res_dir / "rand-prac.npy", cvMD_rand)
        ...
        if not op.exists(res_dir / "pat-prac.npy") or overwrite:
            ...
            np.save(res_dir / "pat-prac.npy", cvMD_pat)
    ...
    for region in ['Hippocampus', 'Thalamus', 'Cerebellum-Cortex']:
        ...
        if not op.exists(res_dir / "rand-learn.npy") or overwrite:
            ...
            cvMD_rand = np.array(cvMD_rand).mean(0)
            np.save(res_dir / "rand-learn.npy", cvMD_rand)
            del cvMD_rand, X_train, X_test, y_train, y_test, data_cov, noise_cov, rank, filters, stcs_train, label_tc, stcs_data
            gc.collect()
        ...
        if not op.exists(res_dir / "pat-learn.npy") or overwrite:
            ...
            cvMD_pat = np.array(cvMD_pat).mean(0)
            np.save(res_dir / "pat-learn.npy", cvMD_pat)
            del cvMD_pat, X_train, X_test, y_train, y_test, data_cov, noise_cov, rank, filters, stcs_train, label_tc, stcs_data
            gc.collect()
