...
import pandas as pd
from base import ensure_dir, ensured, reorder
from config import *
import gc
import sys
...
def process_subject(subject, mode_ICA, filtering, overwrite, jobs, verbose):      
        ...
        session_num = 0
        behav_session = behav_sessions[session_num]
        meg_session = meg_sessions[session_num]
        ...
        if not op.exists(op.join(res_path, 'epochs', f'{subject}-{session_num}-epo.fif')) or overwrite:
                ...
                # equalize behav and events_stim lengths
                changes = editops(int_to_unicode(behav_df['triplets']), int_to_unicode(events_stim[:, 2]))
                if len(changes) !=0:
                        del_from_epoch = list()
                        del_from_behav = list()
                        for change in changes:
                                if change[0] == 'insert':
                                        ...
                                        del_from_behav.append(change[1])
                        events_stim = np.delete(events_stim, del_from_epoch, axis=0)
                ...
                fname_behav = data_path / subject / 'behav_data' / behav_sessions[-1]
                behav = open(fname_behav, 'r')
                lines = behav.readlines()
                column_names = lines[0].split()
                if session_num == 0:
