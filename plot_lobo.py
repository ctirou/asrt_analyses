...
for network in tqdm(networks):
    ...
    for subject in subjects:
        ...
        if subject == 'sub05':
            pat_bsl = np.load(res_path / "pat-1.npy") if network in networks[:-3] else np.load(res_path / "pat-4-1.npy")
            rand_bsl = np.load(res_path / "rand-1.npy") if network in networks[:-3] else np.load(res_path / "rand-4-1.npy")
            for i in range(3):
                pattern[i] = pat_bsl.copy()
                random[i] = rand_bsl.copy()
        pats_blocks.append(np.array(pattern))
