...
ensure_dir(res_path / "pval-all")
if not op.exists(res_path / "pval-all" / "all_pattern-pval.npy") or overwrite:
    print('Computing pval for all patterns')
    pval = gat_stats(all_pats - chance, jobs)
    np.save(res_path / "pval-all" / "all_pattern-pval.npy", pval)
if not op.exists(res_path / "pval-all" / "all_random-pval.npy") or overwrite:
    print('Computing pval for all randoms')
    pval = gat_stats(all_rands - chance, jobs)
    np.save(res_path / "pval-all" / "all_random-pval.npy", pval)
if not op.exists(res_path / "pval-all" / "all_contrast-pval.npy") or overwrite:
    print('Computing pval for all contrasts')
    contrasts = all_pats - all_rands
    pval = gat_stats(contrasts, jobs)
    np.save(res_path / "pval-all" / "all_contrast-pval.npy", pval)
...
for ax, data, title in zip(axs.flat, [all_pats, all_rands], ["pattern", "random"]):
    ...
    pval = np.load(res_path / "pval-all" / f"all_{title.lower()}-pval.npy")
    sig = pval < threshold
...
norm = colors.Normalize(vmin=-0.1, vmax=0.1)
images = []
pval_cont = np.load(res_path / "pval-all" / f"all_contrast-pval.npy")
data, title, pval, vmin, vmax = contrasts, "Contrast (Pattern - Random)", pval_cont, -0.03, 0.03
cmap = 'coolwarm'
...
ax.set_title('Predictive effect per block - diag', fontstyle='italic')
...
ax.legend(frameon=False)
ax.set_title('Predictive effect per block - box', fontstyle='italic')
...
for sub in range(len(subjects)):
    tg = []
    for bins in range(46):
        data = np.diag(contrast_bins[sub, bins])
        tg.append(data[idx_timeg].mean())
    diag_bins.append(np.array(tg))
diag_bins = np.array(diag_bins)

bins = [i for i in range(46)]
cmap = plt.cm.get_cmap('tab20', len(subjects))
fig, ax = plt.subplots(1, 1, figsize=(7, 5), sharex=True, layout='tight')
ax.spines['top'].set_visible(False)
ax.spines['right'].set_visible(False)
...
for i in range(diag_bins.shape[0]):
    ax.plot(bins, diag_bins[i], alpha=0.5, color=cmap(i))
ax.plot(bins, diag_bins.mean(0), lw=3, color='#00A08A', label='Mean')
ax.set_ylabel('Mean RSA effect')
ax.legend(frameon=False)
ax.set_title('Predictive effect per bin - diag', fontstyle='italic')
...
ax.set_xticks(blocks)
ax.set_xticklabels([str(i) for i in range(1, 47)])
for i in range(box_bins.shape[0]):
    ax.plot(blocks, box_bins[i], alpha=0.5, color=cmap(i))
ax.plot(blocks, box_bins.mean(0), lw=3, color='#00A08A', label='Mean')
ax.set_ylabel('Mean RSA effect')
ax.legend(frameon=False)
ax.set_title('Predictive effect per bin - box', fontstyle='italic')