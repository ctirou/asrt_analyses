...
for subject in subjects:
        ...
        for session_num, meg_session, behav_session in zip([1], ['3_EPOCH_1'], ['1_eASRT_Epoch_1.txt']):
                # Read the raw data
                ...
                # Save epochs 
                epochs_stim.save(op.join(path, 'stim', f'{subject}_{session_num+1}_s-epo.fif'), overwrite=True)
                epochs_button.save(op.join(path, 'button', f'{subject}_{session_num+1}_b-epo.fif'), overwrite=True)
                epochs_baseline.save(op.join(path, 'bsl', f'{subject}_{session_num+1}_bl-epo.fif'), overwrite=True)
                # Save behavioral data
                behav_df = stim_df
                behav_df.to_pickle(op.join(path, 'behav', f'{subject}_{session_num+1}.pkl'))