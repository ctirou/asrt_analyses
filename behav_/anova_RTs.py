...
# ------------------- PER BLOCK -------------------
all_block = pd.DataFrame(data=np.insert(all_RTb, 0, np.arange(len(subjects)), axis=1), columns=['sub'] + blocks2)
all_block = pd.melt(all_block, id_vars=['sub'], value_vars=blocks2, var_name='block', value_name='RT')
all_block.insert(1, 'Triplet', np.zeros(len(all_block)))

pattern_block = pd.DataFrame(data=np.insert(pat_RTb, 0, np.arange(len(subjects)), axis=1), columns=['sub'] + blocks2)
pattern_block = pd.melt(pattern_block, id_vars=['sub'], value_vars=blocks2, var_name='block', value_name='RT')
pattern_block.insert(1, 'Triplet', np.zeros(len(pattern_block)))

random_block = pd.DataFrame(data=np.insert(rand_RTb, 0, np.arange(len(subjects)), axis=1), columns=['sub'] + blocks2)
random_block = pd.melt(random_block, id_vars=['sub'], value_vars=blocks2, var_name='block', value_name='RT')
