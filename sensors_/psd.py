...
for subject in subjects:
    ...
    #     plt.show()
        ...
        all_events = mne.find_events(raw, shortest_event=1, verbose=verbose)
        ...
        times = [math.trunc(ti) for ti in time_interval]
        unique, counts = np.unique(times, return_counts=True)
        
        for uni, count in zip(unique, counts):
            if uni not in count_dict:
                count_dict[uni] = count
            else:
                count_dict[uni] += count
