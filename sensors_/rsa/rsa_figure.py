...
def sig_span(ax, sig, times):
    for start, end in zip(times[:-1], times[1:]):
        if sig[np.where(times == start)[0][0]]:
            ax.axvspan(start, end, facecolor="#F2AD00", alpha=0.3, edgecolor=None)
...
axd['B1'].plot(times, diff.mean(0), alpha=1, label='Random - Pattern')
axd['B1'].fill_between(times, diff.mean(0) - sem, diff.mean(0) + sem, alpha=0.2)
