...
plt.axhline(0, color='black', linestyle='dashed')
plt.legend()
plt.savefig(op.join(figures_dir, 'low_high_correlations.pdf'), transparent=True)
plt.close()
...
#     all_pvalues.append(spearmanr(learning_index_df["4"], diff_learn[:, -1, t])[1])
# plt.subplots(1, 1, figsize=(14, 5))
...
#     rhos = []
#     for t in range(len(times)):
#         rhos.append(spearmanr(learning_index_df.iloc[sub, 1:], diff_learn[sub, :, t])[0])
