...
def process_subject(subject, trial_type, jobs):
    ...
    # epo_fnames = [epo_dir / f'{f}' for f in sorted(os.listdir(epo_dir)) if '.fif' in f\
    #     and subject in f and not f.startswith('.') and ('pat' in f if trial_type == 'pattern' else 'rand' in f)]
    ...
    epoch = mne.concatenate_epochs(all_epo, verbose=verbose).crop(-0.2, 0.6)
    ...
    res_dir = RESULTS_DIR / 'decoding' / 'sensors' / lock / trial_type / analysis
    # res_dir = RESULTS_DIR / 'decoding' / 'sensors' / lock / trial_type / 'no-bsling'
