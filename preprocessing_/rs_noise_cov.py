...
def process_subject(subject, mode_ICA, generalizing, filtering, overwrite, jobs, verbose):        
        ...
        ensure_dir(res_path / "resting_state")
        
        # Loop across sessions
        for session_num, meg_session in enumerate(['1_RESTING_1', '7_RESTING_2']):
                # Read the raw data
                ...
                if not op.exists(res_path / "resting_state" / f"{subject}-rs{session_num+1}-noise-cov.fif") or overwrite:
                        noise_cov_fname = res_path / "resting_state" / f"{subject}-rs{session_num+1}-noise-cov.fif"
                        ...
                        noise_cov = mne.compute_raw_covariance(raw, tmin=0, tmax=None, reject=reject, picks=picks, rank='info', method='auto', n_jobs=jobs, verbose=verbose)
