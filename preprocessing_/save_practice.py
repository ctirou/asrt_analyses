...
for subject in subjects:
        ...
        behav_session = sorted([f for f in behav_files_filter if '_eASRT_Practice' in f])[0]
        ...
        if subject == 'sub11': # sub11 has wrong triggers so we need to read a txt file with correct events
        ...
        # Save epochs 
        epochs_stim.save(op.join(path, 'stim', f'{subject}-0-epo.fif'), overwrite=overwrite)
        epochs_bsl.save(op.join(path, 'bsl', f'{subject}-0-epo.fif'), overwrite=overwrite)
        epochs_button.save(op.join(path, 'button', f'{subject}-0-epo.fif'), overwrite=overwrite)
